...
export default function MonthlySummary({ expenses }: Props) {
  const months = useMemo(() => {
    return REF.meses.map((name, i) => {
      ...
      const fijo = exps.filter(e => e.proposito === 'Fijo').reduce((s, e) => s + e.amount, 0);
      const puntual = exps.filter(e => e.proposito === 'Puntual').reduce((s, e) => s + e.amount, 0);
      const viajes = exps.filter(e => e.proposito === 'Viajes').reduce((s, e) => s + e.amount, 0);
      const vida = exps.filter(e => e.proposito === 'Nivel de Vida').reduce((s, e) => s + e.amount, 0);
      const inversion = exps.filter(e => e.proposito === 'Inversión').reduce((s, e) => s + e.amount, 0);
